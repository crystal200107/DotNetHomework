    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                List<Order> test1 = orderService.GetByCustomer("李四");
                foreach (Order i in test1) { Console.WriteLine(i); }
                Console.WriteLine("\n");

                List<Order> test2 = orderService.GetByGoods("瓜子");
                foreach (Order i in test2) { Console.WriteLine(i); }
                Console.WriteLine("\n");

                List<Order> test3 = orderService.GetByMerchant("淘宝");
                foreach (Order i in test3) { Console.WriteLine(i); }
                Console.WriteLine("\n");

                List<Order> test4 = orderService.GetByPrice(100);
                foreach (Order i in test4) { Console.WriteLine(i); }
                Console.WriteLine("\n");
